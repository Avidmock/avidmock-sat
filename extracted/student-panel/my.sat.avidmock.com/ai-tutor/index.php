...
.cs-modal {
    ...
    max-width: 420px;
    box-shadow:
        0 0 0 1px rgba(31,226,144,.12),
        0 28px 70px rgba(6,18,16,.32),
        0 6px 18px rgba(6,18,16,.1);
    overflow: hidden;
    animation: csModalIn .5s cubic-bezier(.16,1,.3,1) .05s both;
}
@keyframes csModalIn {
...
.cs-band::before {
    ...
    background:radial-gradient(circle,rgba(31,226,144,.09) 0%,transparent 65%);
...
.cs-chip-dot { width:5px;height:5px;border-radius:50%;background:var(--ac);animation:sbDot 1.8s ease-in-out infinite; }
@keyframes sbDot { 0%,100%{opacity:1;transform:scale(1)}50%{opacity:.3;transform:scale(.5)} }
.cs-title {
    position:relative; z-index:1;
    font-size:1.25rem; font-weight:800; color:#fff;
    letter-spacing:-.03em; line-height:1.25; margin-bottom:7px;
}
.cs-sub {
    position:relative; z-index:1;
    font-size:.8125rem; color:rgba(255,255,255,.36); line-height:1.6;
}
...
.cs-feat-ico svg { width:14px;height:14px;stroke:var(--dk);fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round; }
.cs-feat-ico.ac   { background:rgba(31,226,144,.08); } .cs-feat-ico.ac   svg { stroke:var(--ac2); }
.cs-feat-ico.warn { background:rgba(245,158,11,.08); } .cs-feat-ico.warn svg { stroke:var(--warn); }
.cs-feat-title { font-size:.8125rem; font-weight:700; color:var(--tx); }
.cs-feat-desc  { font-size:.725rem;  color:var(--tx3); margin-top:1px; line-height:1.4; }
.cs-cta {
...
.cs-cta:hover { background:var(--dk2); transform:translateY(-1px); box-shadow:0 6px 16px rgba(20,50,48,.14); }
.cs-cta svg { width:14px;height:14px;stroke:var(--ac);fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round; }
...
<div class="cs-backdrop" id="csBackdrop" role="dialog" aria-modal="true" aria-labelledby="csTitle">
    <div class="cs-modal" id="csModal">
        ...
        <div class="cs-band">
            ...
            <div class="cs-chip">
                <span class="cs-chip-dot"></span>
                Coming Soon
            </div>
            <div class="cs-title" id="csTitle">AI Tutor</div>
            <div class="cs-sub">Your personal SAT coach is almost here — powered by AI, built for your weak spots.</div>
        </div>
        ...
        <div class="cs-body">
            <div class="cs-features">
                <div class="cs-feat">
                    ...
                    <div>
                        <div class="cs-feat-title">Step-by-Step Explanations</div>
                        <div class="cs-feat-desc">Ask any SAT question and get a clear breakdown tailored to your exact level.</div>
                    </div>
                </div>
                <div class="cs-feat">
                    <div class="cs-feat-ico">
                        <svg viewBox="0 0 24 24"><path d="M2 3h6a4 4 0 014 4v14a3 3 0 00-3-3H2z"/><path d="M22 3h-6a4 4 0 00-4 4v14a3 3 0 013-3h7z"/></svg>
                    </div>
                    <div>
                        <div class="cs-feat-title">Math &amp; Reading Focused</div>
                        <div class="cs-feat-desc">Switch between subjects instantly. Supports equations, reading passages, grammar, and more.</div>
                    </div>
                </div>
                <div class="cs-feat">
                    ...
                    <div>
                        <div class="cs-feat-title">Saved Conversation History</div>
                        <div class="cs-feat-desc">Every session is saved so you can revisit explanations and track your learning over time.</div>
                    </div>
                </div>
            </div>
            <a href="/" class="cs-cta">
...
function shake(){
    ...
    m.style.transform  = 'scale(.975) translateY(5px)';
    setTimeout(function(){
        m.style.transition = 'transform .45s cubic-bezier(.16,1,.3,1)';
        m.style.transform  = 'scale(1) translateY(0)';
        setTimeout(function(){ m.style.transition = ''; }, 450);
    }, 80);
}
var backdrop = document.getElementById('csBackdrop');
if (backdrop) {
    backdrop.addEventListener('click', function(e){ if (e.target === this) shake(); });
}
document.addEventListener('keydown', function(e){ if (e.key === 'Escape') shake(); });

/* Focus trap */
var modal    = document.getElementById('csModal');
var focusEls = modal ? modal.querySelectorAll('a[href], button:not([disabled])') : [];
